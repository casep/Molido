...
int totCorrectFaces = 0;
int totUnknowFaces = 0;
int posPositiveCnt = 0;
int posNegativeCnt = 0;
int negPositiveCnt = 0;
int negNegativeCnt = 0;
...
char *argv[];
{
  ...
  backprop_face(trainlist, test1list, test2list, epochs, savedelta, netname,
		list_errors);
printf(" Total numbers of faces been correctly recognized as known faces -> %d \n", posPositiveCnt);
printf(" Total numbers of faces been mistakenly recognized as Unknown faces -> %d \n",posNegativeCnt);
printf(" Total numbers of faces been correctly recognized as Unknown faces -> %d \n", negNegativeCnt);
...
char *netname;
{
  ...
  for (epoch = 1; epoch <= epochs; epoch++) {
    ...
    for (i = 0; i < train_n; i++) {
      ...
      bpnn_train(net, 0.5, 0.5, &out_err, &hid_err);
  ...
  for ( j=1; j<21; j++){
   ...
   /*** Else, the target unit is off... ***/
     ...
     /*** else, we correctly realized that it wasn't me ***/
       ...
       //negNegativeCnt=(negNegative/2)+1;
     }
   }
   //negNegativeCnt = negNegativeCnt + (count--);
  }
//negNegativeCnt = negNegativeCnt + (count--);
return returnFlag;
}
